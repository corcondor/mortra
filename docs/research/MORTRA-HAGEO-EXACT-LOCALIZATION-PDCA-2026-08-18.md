...
**2026-08-19追試による訂正:** この結果は深さの効果を棄却していない。
HAGeo論文は1試行6 roundを独立に2048/8192回行うのに対し、上表は各層で
beamを12本へ縮約する深さ2探索だった。同一64 seedの独立軌道実験では、
`2002CTSTp25` がN=2で0/64、N=6で1/64となり、48番軌道のnative証明を
二重再生できた。詳細は
`MORTRA-HAGEO-DEPTH-PASSK-EXPERIMENT-2026-08-19.md` に記録した。
...
しても未解決だったが、後日の独立N=6追試では新規証明が得られた。従って現在の
ボトルネックは「候補の質だけ」ではなく、十分な構成深さ、独立軌道数、型付き補題の質、
実行速度の4変数に分解して測る必要がある。
...
関係語彙不足は解消した。一方、単純な関係式化と局所agent追加は追加得点を
生まなかった。候補幅・深さについては、2026-08-19の論文条件に近い独立N=6追試で
追加証明が得られたため、前日の否定的判断を撤回する。
