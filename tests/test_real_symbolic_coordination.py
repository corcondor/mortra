...
class RealSymbolicCoordinationArtifactTest(unittest.TestCase):
    def test_strict_exchange_has_two_independent_certificates(self) -> None:
        report = json.loads(
            (ROOT / "data" / "real-symbolic-coordination-imo-ag-30-consolidated-2026-08-16.json")
            .read_text(encoding="utf-8")
        )
        self.assertEqual(report["summary"]["baseline_unresolved"], 13)
        self.assertEqual(report["summary"]["translated"], 6)
        self.assertEqual(report["summary"]["translation_unsupported"], 7)
        self.assertEqual(report["summary"]["strict_exchange_proved_names"], ["2012_p5"])
        result = report["results"]["2012_p5"]
