"""Run the exact slip evaluation (protocol in exact_slip.py) and summarise it.

    python -m experiments.task_agent.run_exact_slip --output reports/task-agent-exact-slip [--slips ...]
...
def run(output, slips, seeds):
    output = Path(output)
    output.mkdir(parents=True, exist_ok=True)
    tag = "-".join(str(s) for s in slips)
    path = output/f"tasks-{tag}.csv"
    fields = ["slip", "seed", "task_id", "task_type", "planner", "p_success", "expected_cost",
              "expected_steps_given_success", "true_product_states", "learned_states"]
    with open(path, "w", newline="", encoding="utf-8") as handle:
        ...
        for seed in seeds:
            ...
            for slip in slips:
                learner = noisy.train(noisy.SlipEngine(parent["genome"], slip,
                ...
                for task_id, (task_type, start, spec) in enumerate(tasks):
                    for row in exact_slip.task_rows(seed, slip, task_id, task_type, start, spec,
                        ...
                        writer.writerow(row)
                handle.flush()
                say(f"world {seed} slip {slip}: {len(tasks)} tasks, {len(states)} true states, "
                    f"{len(learner.i2s)} learned, {time.perf_counter()-started:.1f}s")
    say(f"wrote {path}")
    return path
...
def summarise(output):
    ...
    for r in rows:
        groups[(float(r["slip"]), r["planner"])].append(r)
    for (slip, planner), items in sorted(groups.items()):
        table.append({"slip": slip, "planner": planner, "tasks": len(items),
                      ...
                      "mean_expected_cost": round(sum(r["expected_cost"] for r in items)/len(items), 4)})
    by = defaultdict(dict)
    for r in rows:
        by[(float(r["slip"]), r["seed"], r["task_id"])][r["planner"]] = r
    pairs = []
    ...
    for slip in sorted({k[0] for k in by}):
        for a, b in comparisons:
            wins = losses = ties = 0
            diff = 0.0
            per_world = defaultdict(lambda: [0, 0])
            n = 0
            for (s, seed, task_id), per in by.items():
                if s != slip or a not in per or b not in per:
                    continue
                n += 1
                ...
                if abs(ca-cb) < 1e-9:
                    ...
                    losses += 1
                    per_world[seed][1] += 1
            pairs.append({"slip": slip, "a": a, "b": b, "tasks": n,
                          ...
                          "per_world_a_cheaper_b_cheaper": {k: v for k, v in sorted(per_world.items())}})
...
def main():
    ...
    if arguments.summarise:
        ...
        return
    run(arguments.output, arguments.slips, arguments.seeds)
