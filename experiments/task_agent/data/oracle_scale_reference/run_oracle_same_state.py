...
counts=Counter(); bytype=defaultdict(Counter); regret=defaultdict(list)
# categories: generic optimal? current optimal? current improves true d over generic?
    ...
    with zipfile.ZipFile(zp) as z:
        for name in z.namelist():
            ...
            for raw in z.open(name):
                row=json.loads(raw)
                if row.get('policy')!='task_virtual_frontier' or not row.get('virtual_field_decision'): continue
                seed=int(row['seed']); tid=int(row['task_id']); tt=row['task_type']
                task,orc=get(seed,tid)
                state=ast.literal_eval(row['world_state']); memory=ast.literal_eval(row['memory'])
                oa,vals=orc.action(state,memory)
                ga=int(row['generic_counterfactual_action']); ta=int(row['task_action'])
                # true continuation distances after each current action; lower is better
                dg=vals[ga]; dt=vals[ta]; do=vals[oa]
                counts['decisions']+=1; bytype[tt]['decisions']+=1
                if row.get('task_signal_available'): counts['signal']+=1; bytype[tt]['signal']+=1
                if ga==oa: counts['generic_matches_oracle']+=1; bytype[tt]['generic_matches_oracle']+=1
                if ta==oa: counts['task_matches_oracle']+=1; bytype[tt]['task_matches_oracle']+=1
                if ta!=ga:
                    counts['field_changed']+=1; bytype[tt]['field_changed']+=1
                    if dt<dg: counts['changed_improves_oracle_distance']+=1; bytype[tt]['changed_improves_oracle_distance']+=1
                    elif dt>dg: counts['changed_worsens_oracle_distance']+=1; bytype[tt]['changed_worsens_oracle_distance']+=1
                    else: counts['changed_ties_oracle_distance']+=1; bytype[tt]['changed_ties_oracle_distance']+=1
                if row.get('task_signal_available'):
                    if dt<dg: counts['signal_task_better_true']+=1; bytype[tt]['signal_task_better_true']+=1
                    elif dt>dg: counts['signal_task_worse_true']+=1; bytype[tt]['signal_task_worse_true']+=1
                    else: counts['signal_task_same_true']+=1; bytype[tt]['signal_task_same_true']+=1
                if dg<10**8 and dt<10**8:
                    regret['generic'].append(dg-do); regret['task'].append(dt-do)

out={'counts':dict(counts),'bytype':{k:dict(v) for k,v in bytype.items()},'mean_one_step_oracle_regret':{k:sum(v)/len(v) for k,v in regret.items()},'n_regret':{k:len(v) for k,v in regret.items()}}
print(json.dumps(out,indent=2))
json.dump(out,open('/mnt/data/oracle_same_state_analysis.json','w'),indent=2)
