...
with open('/mnt/data/fresh70_episodes.csv') as f:
    for r in csv.DictReader(f):
        saved[(int(r['seed']),int(r['task_id']),r['policy'])]=(r['success']=='True',int(r['capped_steps']))

# Analyze all registration-ready worlds by default; optional env var permits smoke subset.
ready=[]
for d in sorted(ROOT.iterdir()):
    if not d.is_dir(): continue
    regp=d/'registration.json'
    if not regp.exists(): continue
    reg=json.load(open(regp))
    if reg.get('status')=='ready': ready.append(int(d.name))
limit=os.environ.get('ORACLE_LIMIT')
if limit: ready=ready[:int(limit)]
rows=[]
for wi,seed in enumerate(ready,1):
    eng,snap,tasks=load_seed(seed)
    for td in tasks:
        task=task_from(td['spec'])
        suc,steps,ex,ch=run_oracle_episode(snap,eng,task,td['start'])
        gs=saved[(seed,td['task_id'],'virtual_frontier')]
        cs=saved[(seed,td['task_id'],'task_virtual_frontier')]
        rows.append({'seed':seed,'task_id':td['task_id'],'task_type':td['task_type'],'oracle_success':suc,'oracle_steps':steps,'oracle_exploration':ex,'oracle_changes_vs_generic':ch,'generic_success':gs[0],'generic_steps':gs[1],'current_success':cs[0],'current_steps':cs[1]})
    print('done',wi,len(ready),seed,flush=True)

def summarize(rows):
    import statistics
    n=len(rows)
    out={'worlds':len(set(r['seed'] for r in rows)),'tasks':n}
    for name,key in [('generic','generic_steps'),('current','current_steps'),('oracle','oracle_steps')]:
        out[name]={'success':sum(r[name+'_success'] if name!='oracle' else r['oracle_success'] for r in rows),'mean_steps':sum(r[key] for r in rows)/n}
    for label,a,b in [('oracle-generic','oracle_steps','generic_steps'),('oracle-current','oracle_steps','current_steps'),('current-generic','current_steps','generic_steps')]:
        ds=[r[a]-r[b] for r in rows]
        out[label]={'mean_delta':sum(ds)/n,'median_delta':statistics.median(ds),'wins':sum(d<0 for d in ds),'losses':sum(d>0 for d in ds),'ties':sum(d==0 for d in ds)}
    out['oracle_exploration_decisions']=sum(r['oracle_exploration'] for r in rows)
    out['oracle_changed_vs_generic']=sum(r['oracle_changes_vs_generic'] for r in rows)
    # world-level deltas
    worlds=[]
    for seed in sorted(set(r['seed'] for r in rows)):
        rr=[r for r in rows if r['seed']==seed]
        worlds.append({'seed':seed,'oracle_minus_generic':sum(r['oracle_steps']-r['generic_steps'] for r in rr)/len(rr),'oracle_minus_current':sum(r['oracle_steps']-r['current_steps'] for r in rr)/len(rr)})
    out['worlds_detail']=worlds
    return out
res=summarize(rows)
print(json.dumps(res,indent=2))
json.dump({'summary':res,'rows':rows},open('/mnt/data/oracle_headroom_result.json','w'),indent=2)
