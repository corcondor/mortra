...
def oracle_source_action(L,state,task,memory,orc):
    u0=L.state_to_id[tuple(state)]; initial=(u0,task.advance(memory,state)); states=[initial]; index={initial:0}; actions=[]; virtual=[]; qd=deque([initial]); A=L.num_actions
    while qd:
        u,m=qd.popleft(); row={}
        for a in range(A):
            visits=L.action_visits.get((u,a),0)
            ...
            if not counts: raise RuntimeError('tried without counts')
            v=max(counts,key=lambda x:(counts[x],-x)); z=(v,task.advance(m,L.id_to_state[v]))
            if z not in index: index[z]=len(states); states.append(z); qd.append(z)
            row[a]=index[z]
        actions.append(row)
    nr=len(states); rr=[];cc=[];ww=[]
    for i,row in enumerate(actions):
        for a,j in list(row.items()):
            ...
            rr.append(i);cc.append(j);ww.append(1/A)
    if not virtual:
        return min(range(A),key=lambda a:(L.action_visits.get((u0,a),0),a))
    n=nr+len(virtual); K=csr_matrix((ww,(rr,cc)),shape=(n,n)); src=np.zeros(n)
    for i,(u,m,a) in enumerate(virtual,nr):
        ws=L.id_to_state[u]; su=orc.ids[tuple(ws)]; v=orc.edges[su][a]; ns=orc.states[v]; mn=task.advance(m,ns); j=orc.pids[(v,mn)]; d=orc.dist[j]
        src[i]=0.0 if d>=10**8 else Q**d
    vals=splu(identity(n,format='csc')-Q*K.tocsc()).solve(src)
    row=actions[0]; return max(row,key=lambda a:(float(vals[row[a]]),-a))

def run_episode_oracle_source(snapshot,engine,task,start):
    L=copy.deepcopy(snapshot); P=Planner(); state=tuple(start); memory=task.advance(task.initial_memory,state); L.get_or_add_id(state); orc=TrueOracle(engine,snapshot.id_to_state,task)
    steps=0; explore=0
    while steps<CAP:
        if task.accepting(memory): return True,steps,explore
        model=P.build(L,task,state,memory); a=P.choose(L,task,state,memory,model)
        if a is None:
            a=oracle_source_action(L,state,task,memory,orc); explore+=1
        u=L.get_or_add_id(state); ns=engine.step(state,a); v=L.get_or_add_id(ns); L.record_transition(u,a,v); state=ns; memory=task.advance(memory,state); steps+=1
    return False,CAP,explore

import csv,os,statistics
saved={}
with open('/mnt/data/fresh70_episodes.csv') as f:
    for r in csv.DictReader(f): saved[(int(r['seed']),int(r['task_id']),r['policy'])]=(r['success']=='True',int(r['capped_steps']))
ready=[]
for d in sorted(ROOT.iterdir()):
    if d.is_dir() and (d/'registration.json').exists() and json.load(open(d/'registration.json')).get('status')=='ready': ready.append(int(d.name))
start_i=int(os.environ.get('START_I','0')); end_i=int(os.environ.get('END_I',str(len(ready)))); ready=ready[start_i:end_i]
rows=[]
for wi,seed in enumerate(ready,1):
    eng,snap,tasks=load_seed(seed)
    for td in tasks:
        task=task_from(td['spec']); suc,steps,ex=run_episode_oracle_source(snap,eng,task,td['start']); g=saved[(seed,td['task_id'],'virtual_frontier')]; c=saved[(seed,td['task_id'],'task_virtual_frontier')]
        rows.append({'seed':seed,'task_id':td['task_id'],'task_type':td['task_type'],'oracle_source_success':suc,'oracle_source_steps':steps,'generic_steps':g[1],'current_steps':c[1]})
    print('done',wi,len(ready),seed,flush=True)
res={}
for ref in ['generic','current']:
    ds=[r['oracle_source_steps']-r[ref+'_steps'] for r in rows]; res['oracle_source-'+ref]={'mean':sum(ds)/len(ds),'median':statistics.median(ds),'wins':sum(x<0 for x in ds),'losses':sum(x>0 for x in ds),'ties':sum(x==0 for x in ds)}
res['oracle_source']={'success':sum(r['oracle_source_success'] for r in rows),'n':len(rows),'mean_steps':sum(r['oracle_source_steps'] for r in rows)/len(rows)}
print(json.dumps(res,indent=2)); json.dump({'summary':res,'rows':rows},open(os.environ.get('OUT','/mnt/data/oracle_source_linear_result.json'),'w'),indent=2)
