...
WHAT KIND OF OBJECT THIS IS, STATED BEFORE ANYTHING IS CLAIMED ABOUT IT

A holonomic program is annihilated by an operator `sum_j c_j(x) D^j` with
POLYNOMIAL coefficients over the rationals. Writing the jet as
`Phi = (f, f', ..., f^(r-1))` gives `Phi'(x) = A(x) Phi(x)` with `A` a companion
matrix of RATIONAL FUNCTIONS, not constants. For such an `A`,

    Phi'' = (A' + A^2) Phi

and NOT `A^2 Phi`. Only when `A` is constant do the higher derivatives become
powers of one matrix, and this module never assumes that: nothing here raises a
coefficient matrix to a power as a stand-in for repeated differentiation.

What IS finite-dimensional and is what the module actually stores is the other
side of Stanley's correspondence: a D-finite series has P-RECURSIVE
coefficients. Reading the operator coefficientwise turns differentiation into an
index shift and gives

    sum_{d=0}^{order} P_d(n) a_{n-d} = 0

with `P_d` polynomials in the INDEX `n`. The window `J_m = (a_{m-order}, ...,
a_{m-1})` advances by `J_{m+1} = B(m) J_m`, and `B(m)` depends on `m`: it is a
family of matrices, one per index, not a single matrix being powered.
`companion_matrix(record, index)` takes that index for exactly this reason.

    coefficient field   the rationals. `operator_coefficients` clears the
                        operator to polynomials over QQ and the recurrence
                        inherits that.
    singular indices    `P_0(n)` can vanish at integer `n`. There the recurrence
                        does not determine `a_n`, so those coefficients are not
                        derived: `_first_solvable_index` finds the largest such
                        root and everything below it is stored as a seed.
    initial values      the seeds, exactly `first_solvable_index` of them.
    expansion point     0.

`f^(n)(0) = n! a_n`, so every derivative value at the expansion point is
reachable from the stored data and none of them needs the derivative primitive.

WHAT IS STORED, AND WHAT IS NOT

Stored: the recurrence polynomials, the first solvable index, the seeds. NOT
stored: any derivative value. The use-time route reads only the record.
...
the same series by an independent route -- closure arithmetic on the program
tree, never the annihilating operator. A mismatch refuses the acquisition.

A NOTE ON WHAT THIS IS WORTH

`holonomic_route_discovery.coefficients` already computes these coefficients and
already takes no derivative. Measured against it rather than against repeated
symbolic differentiation, this representation removes no computation; see the
`attribution` block of any differential comparison. What the surrounding
machinery does here is find, certify and reuse an existing route automatically,
which is a different thing from making one faster, and is reported as the
