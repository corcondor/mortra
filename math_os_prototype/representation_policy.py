...
def _first_counterexample(verdict):
    ...
    for check in verdict.get("checks", []):
        witness = check.get("counterexample")
        if witness:
            # the whole witness, so a later reader can re-run it: which layer,
            # which two words, which label, and what each of them gave
            return {"check": check["name"], **witness}
    return None
