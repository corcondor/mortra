...
LEXICON: tuple[DefinitionEntry, ...] = (
    ...
    # ── 3次元 ──────────────────────────────────────────
    # Point3 は SortSpec に既にあるが、LEXICON には定義エントリが無かった。
    # そのため生成器は平面しか扱えなかった。ここから立体を型として持つ。
    # 平面の型を壊さないよう、3次元は別の名前で並置する。
    DefinitionEntry(
        "Point3",
        ("空間の点", "point3", "spatial point"),
        "sort",
        "Sort",
        "Point3 := Vector(Real, 3)",
        "linear_real_arithmetic",
    ),
    DefinitionEntry(
        "Plane3",
        ("平面", "plane"),
        "definition",
        "Point3 x Point3 -> Region3",
        "Plane3(P,n) := {X:Point3 | inner(X - P, n) = 0}",
        "linear_real_arithmetic",
    ),
    DefinitionEntry(
        "Region3",
        ("立体領域", "solid region", "region3"),
        "sort",
        "Sort",
        "Region3 := Subset(Point3)",
        "real_closed_fields",
    ),
    DefinitionEntry(
        "Extrude",
        ("押し出し", "extrude"),
        "definition",
        "Region2 x Real -> Region3",
        "Extrude(S,h) := {(x,y,z):Point3 | (x,y) in S and 0 <= z <= h}",
        "real_closed_fields",
    ),
    DefinitionEntry(
        "Revolve",
        ("回転体", "revolve", "solid of revolution"),
        "definition",
        "Region2 x Line2 x Real -> Region3",
        "Revolve(S,axis,theta) := union over t in [0,theta] of Rotate3(S,axis,t)",
        "real_closed_fields",
    ),
    DefinitionEntry(
        "Rotate3",
        ("空間回転", "rotate3"),
        "definition",
        "Point3 x Line3 x Real -> Point3",
        "Rotate3(P,L,t) := R_{L,t} P with R orthogonal, det R = 1",
        "linear_real_arithmetic",
    ),
    DefinitionEntry(
        "Line3",
        ("空間の直線", "line3"),
        "definition",
        "Point3 x Point3 -> Curve3",
        "Line3(A,B) := {A + s(B - A) | s:Real}",
        "linear_real_arithmetic",
    ),
    DefinitionEntry(
        "Curve3",
    ...
    DefinitionEntry(
        ...
        "sort",
        "Sort",
        "Surface := Subset(Point3) with local chart to Region2",
        "real_closed_fields",
    ),
    DefinitionEntry(
        "Boundary",
        ...
        "definition",
        "Region3 -> Surface",
        "Boundary(V) := closure(V) minus interior(V)",
        "real_closed_fields",
    ),
    DefinitionEntry(
        "CrossSection",
        ("断面", "cross section"),
        "definition",
        "Region3 x Region3 -> Region2",
        "CrossSection(V,H) := chart_2(V and H) for H a plane",
        "real_closed_fields",
    ),
    DefinitionEntry(
        "Loft",
        ("ロフト", "loft"),
        "definition",
        "Region2 x Region2 x Real -> Region3",
        "Loft(S0,S1,h) := {((1-t)p + t q, t h) | p:S0, q:S1, t in [0,1]}",
        "real_closed_fields",
    ),
)
