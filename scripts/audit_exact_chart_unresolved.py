...
def audit_unresolved(union_path: Path, dataset_path: Path) -> dict[str, object]:
    ...
    for name in unresolved:
        result = certify_jgex_with_exact_chart_portfolio(
            sources[name], include_diagram=False
        )
        selected = result.selected
        results[name] = {
            "matched": result.solved,
...
def main() -> int:
    ...
    report = audit_unresolved(args.union, args.dataset)
