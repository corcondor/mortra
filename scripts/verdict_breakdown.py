...
for p in payload["problems"]:
    try:
        ...
        if not exprs:
            ...
            r = solve_with_routing(p.get("body", ""), exprs)
...
def main() -> int:
    ...
    for i in range(0, n, BATCH):
        sys.stderr.write(f'\r{i}/{n}   ')
        sys.stderr.flush()
        rows.extend(run_batch(problems[i:i + BATCH]))
    ...
    numsup = verdicts['numerically_supported']
    print(f'\n  現行 bucket()   certified_correct {proved}   certified_wrong '
          f'{disproved + numsup + verdicts["unverified"]}')
    ...
    out = os.path.join(ROOT, 'data', f'verdict-breakdown-{which}.json')
