...
For each task, in order: re-prove the premise, enumerate candidates from the
grammar, put every one to the closure prover and then to the certificate for
that task, collect candidates that span the same space into one, measure one
representative of each on a probe, choose by Pareto frontier, answer the task,
and check against independent enumeration at the lengths where that is possible.
...
def ascii_page(traces, reuse):
    ...
    for name, trace in traces.items():
        ...
        lines.append("     |")
        lines.append(f"     v  grammar, in its own order, offers "
        ...
        if learned:
            ...
            lines.append("     |   nothing learned: the concrete state is merged "
                         "with itself, which needs no certificate")
