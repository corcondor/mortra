...
def break_evens(*, acquisition_primitive_calls, representation_bits,
    ...
    """Three of them, kept apart, because they are paid in different currencies.

    They are not combined into one figure. `total_break_even_reuse_count` is the
    LATER of the two, not a weighted blend: turning bits and calls into one
    number would need an exchange rate between them, and there is none.
    """
    def ceiling(cost, saving):
        return -(-cost // saving) if saving and saving > 0 else None

    compute = ceiling(acquisition_primitive_calls, saving_against_naive)
    against_best = ceiling(acquisition_primitive_calls,
                           saving_against_best_baseline)
    description = ceiling(representation_bits, bits_saved_per_task)
    total = (None if compute is None or description is None
             else max(compute, description))
    return {
        "compute_break_even_reuse_count": compute,
        "compute_break_even_reuse_count_against_best_baseline": against_best,
        "description_break_even_task_count": description,
        "total_break_even_reuse_count": total,
        ...
        "definitions": {
            "compute_break_even_reuse_count":
                ("reuses until the acquisition primitive calls are repaid, "
                 "measured against the naive route"),
            "compute_break_even_reuse_count_against_best_baseline":
                ("the same, measured against the strongest route available "
                 "WITHOUT this representation. This is the honest one wherever "
                 "such a route exists, and it is absent when the representation "
                 "saves nothing over it"),
            "description_break_even_task_count":
                ("tasks until the bits of the representation are repaid by the "
                 "bits it saves per task"),
            "total_break_even_reuse_count":
                ("the later of the compute and description break-evens -- the "
                 "MAXIMUM, deliberately. Not a sum and not a weighted blend, "
                 "because bits and calls have no exchange rate")},
    }
