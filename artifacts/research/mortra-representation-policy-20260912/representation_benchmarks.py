...
def _search_answer(counts, length, candidates, nodes):
    target = max(counts) if counts else None
    at_target = counts.get(target, 0)
    total = sum(counts.values())
    return {"value": {"target": None if target is None else str(target),
                      "words_at_target": at_target,
                      "distinct_values": len(counts),
                      "total_words": total},
            "sequential_depth": length,
            "candidates_generated": candidates,
            "search_nodes": nodes,
            "rejected_candidates": total - at_target}


def enumerated_search_run(task, length):
    """naive: every word, folded, and the quantity the task reads taken off it."""
    counts, nodes = {}, 0
    for letters in itertools.product(task.alphabet, repeat=length):
        word = "".join(letters)
        state = task.seed_state
        for symbol in word:
            state = task.step(state, symbol)
            nodes += 1
        if task.legal is not None and not task.legal(state, word):
            continue
        value = task.observable_value(state, word)
        counts[value] = counts.get(value, 0) + 1
    return _search_answer(counts, length, len(task.alphabet) ** length, nodes)


def raw_state_merge_run(task, length):
    """generic_search: merge words that reach equal raw states. Nothing learned.

    A standard technique. It is here so the collapse the representation produces
    can be attributed: whatever this arm already achieves is not the
    representation doing it.
    """
    layer, nodes = {tuple(task.seed_state): 1}, 0
    for _ in range(length):
        following = {}
        for state, multiplicity in layer.items():
            for symbol in task.alphabet:
                moved = tuple(task.step(state, symbol))
                nodes += 1
                following[moved] = following.get(moved, 0) + multiplicity
        layer = following
    counts = {}
    for state, multiplicity in layer.items():
        value = task.observable_value(state, None)
        counts[value] = counts.get(value, 0) + multiplicity
    answer = _search_answer(counts, length, len(layer), nodes)
    answer["distinct_raw_states_at_depth"] = len(layer)
    return answer


def represented_search_run(closure, task, length):
    """representation: expansion over observation vectors, multiplicity carried.

    Two words with the same observation are the same problem for this task --
    which is what the certificate established, and only then -- so they are
    merged, and the multiplicity is kept so the answer is still about words.
    """
    matrices = closure["_matrices"]
    size = len(closure["_basis"])
    layer, nodes = {tuple(F.initial_vector(closure)): 1}, 0
    for _ in range(length):
        following = {}
    ...
    answer = _search_answer(counts, length, len(layer), nodes)
    answer["flop_proxy"] = nodes * (size * size + size * (size - 1))
    return answer
...
def fold_search_comparison(name, closure_record, acquisition, *, task, length,
    ...
    A refusal stops the represented arm from running at all. The record is then
    a refusal record: which check refused, with its counterexample, and no
    represented measurement, because there is none to report.
    """
    ...
    naive = E.measure(lambda: enumerated_search_run(task, length))
    memoised = E.measure(lambda: enumerated_search_run(task, length), cold=False)
    generic = E.measure(lambda: raw_state_merge_run(task, length))
    represented = E.measure(lambda: represented_search_run(closure, task, length))

    agree = naive["value"] == represented["value"]
    generic_agrees = generic["value"] == naive["value"]
    heldout = []
    for other in heldout_lengths:
        left = raw_state_merge_run(task, other)["value"]
        right = represented_search_run(closure, task, other)["value"]
    ...
    record = E.evaluate(
        ...
        search_units=("candidates_generated counts what was expanded -- words "
                      "on the naive rung, distinct raw states on the generic "
                      "rung, distinct observation vectors on the represented "
                      "one. search_nodes counts state updates against matrix "
                      "actions. rejected_candidates is in WORDS on every rung, "
                      "because the merging rungs carry the multiplicity"),
        flop_proxy_note=("counted exactly for the matrix route (one action per "
                         "node). Not instrumented for the kinematics, so those "
                         "figures are null rather than a guess"),
        raw_state_dimension=FOLD_STATE_DIMENSION,
        representation_dimension=len(closure_record["basis"]),
        state_dimension_sense=(
            "the generic rung keeps two states apart whenever their frame or "
            "centre differs, even where the question cannot tell them apart. "
            "The observation vector is what the question actually reads."),
        memoisation_only={
            "primitive_calls": memoised["primitive_calls"],
            "fold_step_calls": memoised["fold_step_calls"],
            "wall_time": memoised["wall_time"],
        ...
        agreement={"agree": agree,
                   ...
                   "held_out_lengths": [{"length": n, "agree": ok}
                                        for n, ok in heldout],
                   "compared": "after every run finished, never during"},
        note=("the answer is a count of words on every rung; the merging rungs "
              "carry multiplicity so the three answers are the same object"))
    record["attribution_control"] = {
        "arm": "merging equal raw states -- a standard technique, nothing learned",
        "search_nodes": generic["search_nodes"],
        "candidates_generated": generic["candidates_generated"],
        "distinct_raw_states_at_depth":
            generic.get("distinct_raw_states_at_depth"),
        "distinct_observation_vectors_at_depth":
            represented["candidates_generated"],
