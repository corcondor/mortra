...
test('finite typed search discovers an auxiliary intersection without an LLM', {
  ...
  assert.equal(result.shared_workspace?.policy, 'only DDAR-proved predicates over original problem points are shared')
  assert.ok(Array.isArray(result.shared_workspace?.facts))
  assert.ok(result.shared_workspace?.facts.every(fact => !fact.includes('aux')))
